...
export default function Reports() {
  const { fetchBalanceSheet, fetchIncomeStatement, loadAll, transactions } = useBeancountStore();
  ...
  useEffect(() => {
    ...
    if (reportType === 'balance-sheet') {
      ...
      fetchIncomeStatement(period.start, period.end).then(setIncomeStatementData);
    }
  }, [reportType, period]);
  ...
  const balanceSheet = balanceSheetData || { assets: [], liabilities: [], equity: [] };
  const incomeStatement = incomeStatementData || { income: [], expenses: [] };
  const cashFlow = generateCashFlow();

  const totalAssets = balanceSheet.assets?.reduce((sum: number, a: any) => sum + a.balance, 0) || 0;
  const totalLiabilities = balanceSheet.liabilities?.reduce((sum: number, l: any) => sum + l.balance, 0) || 0;
  const totalEquity = balanceSheet.equity?.reduce((sum: number, e: any) => sum + e.balance, 0) || 0;
  const totalIncome = incomeStatement.income?.reduce((sum: number, i: any) => sum + i.total, 0) || 0;
  const totalExpenses = incomeStatement.expenses?.reduce((sum: number, e: any) => sum + e.total, 0) || 0;
          ...
          <Header
            ...
            actions={
              <SpaceBetween direction="horizontal" size="xs">
                <DateInput
                  value={period.start}
                  onChange={(e) => setPeriod({ ...period, start: e.detail.value })}
                  placeholder="Start date"
                />
                <DateInput
                  value={period.end}
                  onChange={(e) => setPeriod({ ...period, end: e.detail.value })}
                ...
                <Select
                  ...
                  onChange={(e) => setReportType(e.detail.selectedOption.value as any)}
                  options={[
      ...
      >
        {reportType === 'balance-sheet' && (
          <Grid gridDefinition={[{ colspan: 4 }, { colspan: 4 }, { colspan: 4 }]}>
            <Container variant="stacked" header={<Header variant="h3">Assets</Header>}>
              <KeyValuePairs
            ...
            <Container variant="stacked" header={<Header variant="h3">Liabilities</Header>}>
              <KeyValuePairs
                items={(balanceSheet.liabilities || []).map((liability: any) => ({
            ...
            <Container variant="stacked" header={<Header variant="h3">Equity</Header>}>
              <KeyValuePairs
        ...
        {reportType === 'income-statement' && (
          <Grid gridDefinition={[{ colspan: 6 }, { colspan: 6 }]}>
            <Container variant="stacked" header={<Header variant="h3">Income</Header>}>
              <KeyValuePairs
            ...
            <Container variant="stacked" header={<Header variant="h3">Expenses</Header>}>
              <KeyValuePairs
        ...
        {reportType === 'cash-flow' && (
          <Container variant="stacked" header={<Header variant="h3">Cash Flow</Header>}>
